!(function(t,e){"object"==typeof exports&&"undefined"!=typeof module?e(exports,require("@uirouter/core"),require("lit-element"),require("wgnhs-common"),require("wgnhs-layout"),require("wgnhs-viz")):"function"==typeof define&&define.amd?define(["exports","@uirouter/core","lit-element","wgnhs-common","wgnhs-layout","wgnhs-viz"],e):e((t=t||self).app={},t.common,t.common,t.common,t.lit,t.lit)})(this,function(t,e,i,s,o,a){"use strict";const r=["#e0ecf4","#bfd3e6","#9ebcda","#8c96c6","#8c6bb1","#88419d","#810f7c"],n=L.CircleMarker.extend({getEvents:function(){return{zoomend:this._restyle,normalpoints:this._normal,typepoints:this._orifice,condpoints:this._conductivity,qpoints:this._discharge}},_restyle:function(t){this.setRadius(n.calcRadius(t.target.getZoom()))},_normal:function(t){this._activeBackup?this._activeBackup="#3388ff":this.setStyle({color:"#3388ff"})},_orifice:function(t){var e="#3388ff";"seepage/filtration"===this.feature.properties.Orifice_Geom&&(e="#33AA44"),this._activeBackup?this._activeBackup=e:this.setStyle({color:e})},_conductivity:function(t){for(var e=r[r.length-1],i=(aggrData.aggr.Conductivity_uS.max-aggrData.aggr.Conductivity_uS.min)/r.length,s=1;s<r.length;s++)if(i*s+aggrData.aggr.Conductivity_uS.min>this.feature.properties.Conductivity_uS){e=r[s];break}this._activeBackup?this._activeBackup=e:this.setStyle({color:e})},_discharge:function(t){for(var e=r[r.length-1],i=(aggrData.aggr.Discharge_cfs.max-aggrData.aggr.Discharge_cfs.min)/r.length,s=1;s<r.length;s++)if(i*s+aggrData.aggr.Discharge_cfs.min>this.feature.properties.Discharge_cfs){e=r[s];break}this._activeBackup?this._activeBackup=e:this.setStyle({color:e})},highlight:function(){this._activeBackup=this.options.color,this.setStyle({color:"orange"})},removeHighlight:function(){this._activeBackup&&(this.setStyle({color:this._activeBackup}),this._activeBackup=null)}});n.calcRadius=(t=>Math.max(Math.floor(t/1.5),3));class l{constructor(t,e,i){this._springPhotos=e,this._springSketches=i,this._aggrKeys=["Site_Code","Conductivity_uS","Discharge_cfs","pH","Water_Temp_C"],this.aggrData=l._gatherAggrData(t,this._aggrKeys)}async lookupPhotos(t){const e=this._springPhotos;return new Promise(function(i,s){e.query().where("Site_Code = '"+t+"'").run(function(t,e){if(t)s(t);else{var o=e.features.map(function(t){return t.properties});o.sort(function(t,e){return t.Image_Number-e.Image_Number}),i(o)}})})}async lookupSketches(t){const e=this._springSketches;return new Promise(function(i,s){e.query().where("File_Prefix = '"+t+"'").run(function(t,e){if(t)s(t);else{var o=e.features.map(function(t,e){return t.properties.FileURL});i(o)}})})}static _gatherAggrData(t,e){const i={aggr:{},data:[]};return t.eachFeature(function(t,s){let o={};e.forEach(function(e){o[e]=t.feature.properties[e],i.aggr[e]||(i.aggr[e]={}),o[e]&&"number"==typeof o[e]&&((!i.aggr[e].max||i.aggr[e].max<o[e])&&(i.aggr[e].max=o[e]),(!i.aggr[e].min||i.aggr[e].min>o[e])&&(i.aggr[e].min=o[e]))}),i.data.push(o)}),i}}const c=function(t){return t?t.toFixed(2):null},h=["OBJECTID","GlobalID","File_Prefix","Site_Code","SpringID","Surveyor","Time","Horz_Precision_m","Max_PDOP","Access","Ease_Access","Notes","gps_time_date","sat_signals"],d={pH:{title:"pH",desc:"Measured as close to spring source as possible."},Conductivity_uS:{title:"Specific Conductance (µmho/cm)",desc:"Measured as close to spring source as possible (µmho/cm)."},Water_Temp_C:{title:"Temperature (°C)",desc:"Measured as close to spring source as possible (°C)."},SpringID:{title:"Spring ID",desc:"Unique identifier within county."},County:{title:"County",desc:"County where spring is located."},Surveyor:{title:"Surveyor(s)",desc:"Who conducted the survey (initials)."},Date:{title:"Date",desc:"Date of field survey.",transform:function(t){return t?new Date(t).toISOString().substring(0,10):null}},Time:{title:"Time",desc:"Start time."},Easting_WTM:{title:"Easting (WTM)",desc:"Easting (WTM). As close to the spring source as possible.",transform:c},Northing_WTM:{title:"Northing (WTM)",desc:"Northing (WTM). As close to the spring source as possible.",transform:c},Horz_Precision_m:{title:"Horizontal Precision (meters)",desc:"Horizontal accuracy of GPS position (meters)."},Max_PDOP:{title:"Maximum PDOP",desc:"Maximum positional dilution of precision (PDOP) during measurement."},Elevation_m:{title:"Elevation (meters)",desc:"From digital elevation model (DEM) (meters).",transform:c},elevation_source:{title:"Elevation Source",desc:"DEM source and horizontal resolution of DEM used to extract elevation."},Land_Owner:{title:"Land Ownership",desc:"List: state, county, city, NPS, USFS, tribal, military, private, other."},Access:{title:"Access",desc:"Directions to springs."},Ease_Access:{title:"Ease of Access",desc:"List: Easy access, Difficult access, Terrain prohibits access to other potential spring areas."},Land_Cover:{title:"Land Cover",desc:"List: urban, residential, agriculture, grassland, forest, open water, wetland, barren, shrubland, other."},Air_Temp_F:{title:"Air Temperature (°F)",desc:"Air temperature on date surveyed (°F)."},Cloud_Cover_percent:{title:"Cloud Cover (%)",desc:"Cloud cover at time of survey (%)."},Wind_Speed_mph:{title:"Wind Speed (mph)",desc:"Velocity measurement on date surveyed (mph)."},Aspect_degrees:{title:"Aspect",desc:"Direction that the spring orifice faces."},Slope_degrees:{title:"Slope (°)",desc:"Channel slope (°)."},Slope_Variability:{title:"Slope Variability",desc:"List: high, medium, low, none."},Condition:{title:"Condition",desc:"List: undisturbed, light, moderate, high."},Type_of_Disturbance:{title:"Type of Disturbance",desc:"List: wildlife, livestock, recreation, diversion, residence, impounded, dredging, flooding, trails, roadway, invasives, spring house, encased, raceways, human-made structure, trash, storm-water, drain tile, agriculture, other."},Spring_Area_sqm:{title:"Spring Area (m²)",desc:"List: <2 m², 2-10 m², 10-100 m², 100-1000 m², 1000-10,000 m², 10,000-100,000 m²"},Surface_Types:{title:"Surface Type(s)",desc:"List: backwall, colluvial slope, sloping bedrock, pool, channel, spring mound, cave, other."},Width_ft:{title:"Channel or Pool Width (feet)",desc:"If a channel or pool exists, the mean width (feet)."},Width_Location:{title:"Width Location",desc:"List: pool, channel, pond, spring house, other."},Depth_cm:{title:"Channel or Pool Depth (cm)",desc:"If a channel or pool exists, the mean depth (cm)."},Depth_Location:{title:"Depth Location",desc:"List: pool, channel, pond, spring house, other."},Percent_organic:{title:"Percent Organic",desc:"Qualitative estimate of the % organics. Described as close to spring source as possible."},Percent_fines:{title:"Percent Fines",desc:"Qualitative estimate of the % fines. Described as close to spring source as possible."},Percent_sand:{title:"Percent Sand",desc:"Qualitative estimate of the % sand. Described as close to spring source as possible."},Percent_gravel:{title:"Percent Gravel",desc:"Qualitative estimate of the % gravel. Described as close to spring source as possible."},Percent_cobble:{title:"Percent Cobble",desc:"Qualitative estimate of the % cobble. Described as close to spring source as possible."},Percent_boulder:{title:"Percent Boulder",desc:"Qualitative estimate of the % boulder. Described as close to spring source as possible."},Percent_bedrock:{title:"Percent Bedrock",desc:"Qualitative estimate of the % bedrock. Described as close to spring source as possible."},Bedrock_Comp:{title:"Bedrock Composition",desc:"List: shale, siltstone, sandstone, conglomerate, limestone, dolomite, igneous or metamorphic, NA, other."},Spring_Type:{title:"Spring Type",desc:"List: helocrene, rheocrene, limnocrene, hillslope spring, cased, flowing well, other."},Spring_Source:{title:"Spring Source",desc:"List: single orifice, multiple orifices, diffuse flow, other."},Orifice_Geom:{title:"Orifice Geomorphic Type",desc:"List: seepage/filtration, fracture, tubular, contact."},Discharge_cfs:{title:"Discharge (cfs)",desc:"Spring flow (cfs)."},Flow_Accuracy:{title:"Flow Accuracy",desc:"Level of accuracy of flow measurement, List: low, high"},Discharge_Meas:{title:"How Measured",desc:"List: timed volume, float velocity method, flume, AA meter, AD meter (acoustic Doppler meter), EM meter (electromagnetic meter)."},Flow_Location:{title:"Flow Location",desc:"Where flow was measured."},Flow_percent:{title:"Flow %",desc:"Percent of flow captured (%)."},Veg_Bed_Cover_percent:{title:"Vegetative Bed Cover (%)",desc:"The proportion of the spring pool bed or channel bed that is covered by live vegetation (%)."},Veg_Bank_Cover_percent:{title:"Vegetative Bank Cover (%)",desc:"The proportion of the spring pool banks or channel banks that is covered by live vegetation (%)."},Notes:{title:"Notes",desc:"Other notes as necessary."},GlobalID:{title:"Global ID",desc:"Automatically generated unique and global ID"},gps_time_date:{title:"GPS time and date",desc:"Automatically generated GPS time and date stamp"},sat_signals:{title:"Number of satellites",desc:"Automatically generated number of satellites visible"}},u="entry";const p="toggle-pdf-panel";customElements.define("site-sketch-button",class extends i.LitElement{static get properties(){return{src:{type:String},panel:{type:Object,attribute:!1},closed:{type:Boolean,attribute:!1}}}constructor(){super(),this.closed=!0}static get styles(){return i.css`
    ...
    `:""}
    `}firstUpdated(){this.$modal=this.renderRoot.querySelector("modal-dialog")}handleImageClick(t){this.clickedImg=t.target.src,this.$modal.source=this.clickedImg,this.$modal.toggleOpen()}});class g extends i.LitElement{static get properties(){return{siteinfo:{type:Object},photos:{type:Array},pdfpanel:{type:Object},pdfsrc:{type:String},printLayout:{type:Boolean,attribute:"print-layout"}}}constructor(){super(),this.genId=(function(){const t={};return function(e){return t[e]||(t[e]=s.genId()),t[e]}})()}static get styles(){return i.css`
      [data-element="table"] {
        display: grid;
    ...
    <td class="label" title="${d[t[0]]?d[t[0]].desc:t[0]}">
      <label for="${this.genId(e)}" >
        ${d[t[0]]?d[t[0]].title:t[0]}
      </label>
    </td>
    <td class="detail" title="${d[t[0]]?d[t[0]].desc:t[0]}">
      <span id="${this.genId(e)}">
        ${d[t[0]]&&d[t[0]].transform?d[t[0]].transform(t[1]):t[1]}
      </span>
    </td>
  `)}render(){return i.html`
      <style>
      ...
      `:""}
    `}handlePrint(){window.print()}fireClearSelection(){let t=new CustomEvent("clear-selection",{bubbles:!0,detail:{}});this.dispatchEvent(t)}fireTogglePrint(){let t=new CustomEvent("toggle-print",{bubbles:!0,detail:{on:!this.printLayout,params:this.siteinfo}});this.dispatchEvent(t)}}customElements.define("site-details",g);class m extends i.LitElement{static get properties(){return{}}constructor(){super()}static get styles(){return i.css`
    .option-container {
      box-sizing: border-box;
    ...
    `}handleSelection(t){}_fire(t,e){let i=new CustomEvent(t,{bubbles:!0,detail:e||{}});this.dispatchEvent(i)}normalPoints(){this._fire("stylepoints",{type:"normal"})}typePoints(){this._fire("stylepoints",{type:"type"})}condPoints(){this._fire("stylepoints",{type:"cond"})}qPoints(){this._fire("stylepoints",{type:"q"})}}customElements.define("map-controls",m);customElements.define("map-control-item",class extends i.LitElement{static get properties(){return{selected:{type:Boolean}}}constructor(){super(),this.genId=s.genId()}static get styles(){return i.css`
